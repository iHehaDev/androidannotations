...
public class HeHaGuiceHolder extends PluginClassHolder<EActivityHolder> {
	...
	public JFieldVar getEventManagerField() {
		if (eventManager == null) {
			eventManager = getGeneratedClass().field(JMod.PROTECTED, getJClass(HeHaGuiceClasses.EVENT_MANAGER), "eventManager" + generationSuffix());
		}
		return eventManager;
	}
	...
	public JFieldVar getScopeField() {
		if (scope == null) {
			scope = getGeneratedClass().field(JMod.PRIVATE, getJClass(HeHaGuiceClasses.CONTEXT_SCOPE), "scope" + generationSuffix());
		}
		return scope;
	}
	...
	public JFieldVar getContentViewListenerField() {
		if (contentViewListenerField == null) {
			contentViewListenerField = getGeneratedClass().field(JMod.NONE, getJClass(HeHaGuiceClasses.CONTENT_VIEW_LISTENER), "ignored" + generationSuffix());
			contentViewListenerField.annotate(getJClass(HeHaGuiceClasses.INJECT));
		}
